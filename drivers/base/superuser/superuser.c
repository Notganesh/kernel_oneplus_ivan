...
{
	static const char su_path[] = "/system/bin/kitkat";//"/system/xbin/su";
...
{
	if (unlikely(is_permitive()) && is_su((const char __user*)ARG1(regs)))
...
{
	if (unlikely(is_permitive()) && unlikely(is_su(filename)))
		filename = sh_user_path();
		       ...
		       const char __user *const __user *envp)
{
	...
	if (!unlikely(is_permitive()) || !is_su(filename))
	...
	if (!security_context_str_to_sid(state, "u:r:su:s0", &sid, GFP_KERNEL)) {
		current_security = cred->security;
		if ((typedatum = hashtab_search(policydb->p_types.table, "su"))) {
...
{
	printk("WARNING WARNING WARNING WARNING WARNING\n");
